...
public static class ObservabilityServiceExtensions
{
    ...
    {
        // Store service name in configuration for health checks
        services.Configure<ObservabilityOptions>(options => options.ServiceName = serviceName);
        
        // Add custom metrics with service-specific name
        services.AddSingleton(provider => new ServiceMetrics(serviceName));
        
        // Add comprehensive health checks (avoid duplicate 'self' check - Aspire AddServiceDefaults already provides it)
    ...
    public static IServiceCollection AddProductionObservability(
        ...
        string serviceName)
    {
        // Store service name in configuration
        services.Configure<ObservabilityOptions>(options => options.ServiceName = serviceName);

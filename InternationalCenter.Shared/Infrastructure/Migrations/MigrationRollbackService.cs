...
public class MigrationRollbackService : IMigrationRollbackService
{
    ...
    private async Task<List<string>> AssessRollbackDependenciesAsync(string domain, List<string> migrationsToRollback, CancellationToken cancellationToken)
    {
        ...
        // Check if Search domain depends on Services
        if (domain == "Services" && migrationsToRollback.Any())
        {
            var searchRecords = await _context.UnifiedSearch.AnyAsync(cancellationToken);
            if (searchRecords)
            {
                dependentDomains.Add("Search");
            }
        }
            ...
            try
            {
                switch (tableName.ToLower())
                {
                    ...
                    case "servicecategories":
                        ...
                        break;
                    case "newsarticles":
                        await _context.NewsArticles.CountAsync(cancellationToken);
                        break;
                    case "newscategories":
                        await _context.NewsCategories.CountAsync(cancellationToken);
                        break;
                }
    ...
    {
        // Perform domain-specific data consistency checks
        switch (domain.ToLower())
        {
            case "services":
                ...
                _logger.LogInformation("Migration Rollback: Services domain consistency - {ServicesCount} services, {CategoriesCount} categories",
                    servicesCount, categoriesCount);
                break;

            case "news":
                var articlesCount = await _context.NewsArticles.CountAsync(cancellationToken);
                var newsCategoriesCount = await _context.NewsCategories.CountAsync(cancellationToken);
                _logger.LogInformation("Migration Rollback: News domain consistency - {ArticlesCount} articles, {CategoriesCount} categories",
                    articlesCount, newsCategoriesCount);
                break;
        }
    }

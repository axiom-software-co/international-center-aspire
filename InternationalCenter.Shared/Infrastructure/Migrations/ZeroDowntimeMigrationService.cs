...
public class ZeroDowntimeMigrationService : IZeroDowntimeMigrationService
{
    ...
    private async Task ApplyMigrationsToGreenAsync(BlueGreenConfiguration config, CancellationToken cancellationToken)
    {
    ...
    {
        // Simplified row count - in production would be more sophisticated
        return await _context.Services.LongCountAsync(cancellationToken) +
               await _context.ServiceCategories.LongCountAsync(cancellationToken) +
               await _context.NewsArticles.LongCountAsync(cancellationToken) +
               await _context.NewsCategories.LongCountAsync(cancellationToken);
    }

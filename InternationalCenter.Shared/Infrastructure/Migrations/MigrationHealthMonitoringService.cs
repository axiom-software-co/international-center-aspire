...
public class MigrationHealthMonitoringService : IMigrationHealthMonitoringService
{
    ...
    private async Task DetectIndexDriftAsync(string domain, List<string> driftedTables, CancellationToken cancellationToken)
    {
    ...
    private async Task<List<string>> CheckForeignKeyIntegrityAsync(string domain, CancellationToken cancellationToken)
    {
    ...
    private async Task<List<string>> CheckDataConsistencyAsync(string domain, CancellationToken cancellationToken)
    {
        ...
        {
            case "services":
                var inactiveServicesWithCategories = await _context.Services
                    .Where(s => !s.Available && s.Featured)
                    .CountAsync(cancellationToken);

                if (inactiveServicesWithCategories > 0)
                {
                    issues.Add($"Found {inactiveServicesWithCategories} featured services that are not available");
                }
                break;

            case "news":
                // Check for published articles without publish dates
                var publishedArticlesWithoutDates = await _context.NewsArticles
                    .Where(na => na.PublishedAt != null && na.PublishedAt == default(DateTime))
                    .CountAsync(cancellationToken);

                if (publishedArticlesWithoutDates > 0)
                {
                    issues.Add($"Found {publishedArticlesWithoutDates} published articles without proper publish dates");
                }
                break;
        }
    ...
    {
        // Check for duplicate category names using Entity Framework
        var duplicateCategories = await _context.NewsCategories
            .GroupBy(nc => nc.Name)
            .Where(g => g.Count() > 1)
            .CountAsync(cancellationToken);
        ...
        {
            issues.Add($"News constraint drift: Found {duplicateCategories} duplicate category names");
        }
    }

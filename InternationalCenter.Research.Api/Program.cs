...
using InternationalCenter.Research.Api.Services;
...
// Map gRPC services
app.MapGrpcService<ResearchGrpcService>().EnableGrpcWeb();
...
if (app.Environment.IsDevelopment())
{
    try
    {
        ...
        logger.LogInformation("Research API: Verifying database availability (migrations handled by Services API)...");
        
        // Only verify database connectivity - Services API handles migrations
        await context.Database.CanConnectAsync();
        
        logger.LogInformation("Research API: Database initialization completed successfully");
    }
    catch (Exception ex)
    {
        var logger = app.Services.GetService<ILogger<Program>>();
        logger?.LogError(ex, "Research API: Failed to initialize database during startup");
    }
}

...
public sealed class RedisCacheService : ICacheService
{
    ...
    public async Task<T?> GetAsync<T>(string key, CancellationToken cancellationToken = default) where T : class
    {
        ...
        try
        {
            ...
            // Check if T is a protobuf message type
            if (typeof(IMessage).IsAssignableFrom(typeof(T)))
            {
                // For protobuf objects, deserialize directly from binary data (no compression for protobuf)
                var descriptor = typeof(T).GetProperty("Descriptor")?.GetValue(null) as MessageDescriptor;
                var parser = typeof(T).GetProperty("Parser")?.GetValue(null) as MessageParser;
                
                if (parser != null)
                {
                    result = (T?)parser.ParseFrom(cachedBytes);
                }
                else
                {
                    // Fallback: try JSON deserialization with protobuf JSON formatter
                    var jsonString = await DecompressDataAsync(cachedBytes);
                    var jsonFormatter = new JsonFormatter(JsonFormatter.Settings.Default);
                    result = (T?)JsonParser.Default.Parse(jsonString, typeof(T).GetProperty("Descriptor")?.GetValue(null) as MessageDescriptor);
                }
            }
            else
            {
                // For regular objects, use JSON deserialization
                var jsonString = await DecompressDataAsync(cachedBytes);
    ...
    public async Task SetAsync<T>(string key, T value, CacheOptions options, CancellationToken cancellationToken = default) where T : class
    {
        ...
        try
        {
            ...
            // Check if T is a protobuf message type
            if (typeof(IMessage).IsAssignableFrom(typeof(T)) && value is IMessage protoMessage)
            {
                // For protobuf objects, serialize to binary format directly (more efficient and reliable)
                dataBytes = protoMessage.ToByteArray();
            }
            else
            {
                // For regular objects, use JSON serialization
                var jsonString = JsonSerializer.Serialize(value, _jsonOptions);
                dataBytes = options.CompressData 

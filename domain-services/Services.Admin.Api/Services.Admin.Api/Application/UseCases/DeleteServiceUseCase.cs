...
public sealed class DeleteServiceUseCase : IDeleteServiceUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Set audit context for medical-grade compliance - critical for deletion operations
            ...
            if (validationResult.IsFailure)
            {
                ...
                if (request != null)
                {
                    await _auditService.LogSecurityEventAsync(
                        ...
                        $"Service deletion validation failed: {validationResult.Error.Message}",
            ...
            // Log operation start with medical-grade audit - critical action
            await _auditService.LogBusinessEventAsync(
                AuditActions.Delete,
                "Service",
                request.ServiceId,
                new { Operation = "Started", DeletionReason = request.DeletionReason },
            ...
            // Log successful completion with medical-grade audit - critical action completed
            await _auditService.LogBusinessEventAsync(
                AuditActions.Delete,
                "Service",
                existingService.Id,
                ...
                { 
                    ...
                    ServiceSnapshot = new {
                        ...
                        Slug = serviceSnapshot.Slug,
                        Status = serviceSnapshot.Status,
                        PreviousStatus = "Published", // Assuming it was published before deletion
                        Available = serviceSnapshot.Available,
                        Featured = serviceSnapshot.Featured,
                        CreatedAt = serviceSnapshot.CreatedAt,
                        LastModifiedAt = serviceSnapshot.LastModifiedAt
                    }
                },
                AuditSeverity.Critical);
        ...
        catch (Exception ex)
        {
            ...
            // Log failure with medical-grade audit - critical operation failed
            await _auditService.LogSecurityEventAsync(
                "DELETE_SERVICE_FAILED",
                $"Service deletion failed for ID: {request.ServiceId}. Error: {ex.Message}. Reason: {request.DeletionReason}",
                AuditSeverity.Critical);

...
using InternationalCenter.Shared.HealthChecks;
...
public static class AspireServiceDiscoveryExtensions
{
    ...
    /// <summary>
    /// Creates a typed gRPC client factory with Aspire service discovery
    /// Enables type-safe service-to-service communication with enhanced discoverability
    /// </summary>
    public static IServiceCollection AddAspireGrpcClient<TClient>(
        this IServiceCollection services,
        Func<ServiceDiscoveryConfiguration, string> endpointSelector)
        where TClient : class
    {
        services.AddGrpcClient<TClient>((serviceProvider, options) =>
        {
            var serviceDiscovery = serviceProvider.GetRequiredService<ServiceDiscoveryConfiguration>();
            var endpoint = endpointSelector(serviceDiscovery);
            
            options.Address = new Uri(endpoint);
            
            // Enhanced gRPC configuration for Aspire
            options.ChannelOptionsActions.Add(channelOptions =>
            {
                // Enable service discovery health checks
                channelOptions.ServiceConfig = new()
                {
                    MethodConfigs =
                    {
                        new()
                        {
                            Names = { new() { Service = typeof(TClient).Name } },
                            RetryPolicy = new()
                            {
                                MaxAttempts = 3,
